				...
				<form id="document-form" class="content" action="orders.php" method="post">
					...
					<div class="contact">
						...
						<contact class="boxes">
							...
							<div class="input">
								<input type="tel" required placeholder="" maxlength="10" pattern="[0-9]{10}" name="mobile" id="mobile">
						...
						<div class="card-content">
							...
							<div class="total-items">
								<p><?php
									...
									$first_sql = "select sum(quantity) as sum from cart where user_id=$user";
							...
							<div class="list">
								...
								$sql = "select pro_simple_name,quantity,price from cart inner join products on cart.item_id=products.pid where user_id=$user";
								...
								while ($row = mysqli_fetch_assoc($result)) {
									echo "<div class='pro-name'>" . $row['pro_simple_name'] . "</div>" . "<div class='pro-name' style='padding-left:10px'>  (" . $row['quantity'] . ")</div>" . "<div class='pro-name'>₹ " . $row['price'] . "</div>";
								}
								?>
							...
							<div class="total">
								<p>Total</p> <span class="money">
									...
									$next_sql = "select sum(total_price) as sum from cart where user_id=$user";
...
<script>
	...
	const submitToOrder = (object) => {
		...
		if (cartEmpty) {
			return alert("Cart Is empty cannot go forward");
		}
		if (!form1.checkValidity()) {
			...
			i = i + 1;
			if (i > 4) {
				return alert("Please fill the neccesary Data (Type of delivery)");
			}
